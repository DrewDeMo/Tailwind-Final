<div class="clientNav-tabs">
    <div class="mb-8">
        <nav class="clientNav-tab-nav">
            <a href="#" data-tab="clientNav-tab1" class="clientNav-tab-link clientNav-active font-medium pr-1">
                <i class="las la-tachometer-alt text-xl mr-1" style="line-height: 1; vertical-align: bottom;"></i>
                Dashboard
            </a>
            <a href="#" data-tab="clientNav-tab3" class="clientNav-tab-link pr-1"><i class="las las la-tag text-xl mr-1" style="line-height: 1; vertical-align: bottom;"></i>Offers</a>
            <a href="#" data-tab="clientNav-tab2" class="clientNav-tab-link pr-1"><i class="las la-bullhorn text-xl mr-1" style="line-height: 1; vertical-align: bottom;"></i>Global
                Revisions</a>
            <a href="#" data-tab="clientNav-tab5" class="clientNav-tab-link pr-1"><i class="las la-bullseye text-xl mr-1" style="line-height: 1; vertical-align: bottom;"></i>Tracking</a>
            <a href="#" data-tab="clientNav-tab6" class="clientNav-tab-link pr-1"><i class="las la-icons text-xl mr-1" style="line-height: 1; vertical-align: bottom;"></i>Credentials</a>

            <span class="clientNav-indicator"></span>
        </nav>
    </div>
    <div class="clientNav-tab-content">
        <div class="clientNav-tab-pane clientNav-active" id="clientNav-tab1">
            <!--Start Dashboard Content-->
            <div class="flex flex-row justify-between w-full gap-4 mt-4">
                <!--Owners box left-->
                <!-- Owner Operator Information -->
                <section class="relative rounded-lg border w-full bg-white border-gray-300 overflow-hidden">
                    <!-- Background Image and Overlay -->
                    <div class="relative top-0 left-0 right-0 h-16 bg-cover bg-center z-0">
                        <div class="absolute inset-0 opacity-90 clientCardHeadlines"></div>
                        <!-- Headline -->
                        <h2 class="px-4 absolute inset-x-0 bottom-0 h-full flex items-center text-2xl font-bold text-gray-100  z-10">
                            Contacts
                        </h2>
                    </div>


                    <!--Owner 1-->
                    <div class="p-2 mt-2">
                        <div class="flex items-center justify-between w-full hover:bg-gray-50 duration-400 rounded-lg p-2">

                            <div class="flex items-center space-x-2">
                                <!-- Grouped user icon and name -->
                                <div class="h-12 w-12 bg-ww-blue-sub-50 rounded-lg mr-2 flex items-center justify-center contactCard">
                                    <p class="text-lg  font-bold">BH</i>
                                </div>
                                <h3 class="font-medium ml-4">Benjamin Hartley</h3><span class="bg-ww-blue-sub-50 companyTag text-xs font-medium mr-2 px-2.5 py-0.5 rounded">Owner</span>
                            </div>

                            <div class="flex space-x-2">
                                <!-- Two buttons grouped on the right -->
                                <button class="copyText"><i class="las la-copy font-lg border p-1.5 rounded-lg"></i></button>
                                <button><i class="las la-envelope mailButton text-white font-lg bg-ww-blue-500 hover:bg-ww-blue-600 p-1.5 rounded-lg"></i></button>
                            </div>

                        </div>
                    </div>

                    <!--Owner 2-->
                    <div class="p-2">
                        <div class="flex items-center justify-between w-full hover:bg-gray-50 duration-400 rounded-lg p-2">

                            <div class="flex items-center space-x-2">
                                <!-- Grouped user icon and name -->
                                <div class="h-12 w-12 bg-ww-blue-sub-50 rounded-lg mr-2 flex items-center justify-center contactCard">
                                    <p class="text-lg text-ww-blue-sub-500 font-bold">CJ</p>
                                </div>
                                <h3 class="font-medium ml-4">Clara Jeffries</h3><span class="bg-ww-blue-sub-50 companyTag text-xs font-medium mr-2 px-2.5 py-0.5 rounded">Owner</span>
                            </div>

                            <div class="flex space-x-2">
                                <!-- Two buttons grouped on the right -->
                                <button><i class="las la-copy text-gray-600 font-lg border border-gray-200 hover:bg-gray-100 p-1.5 rounded-lg"></i></button>
                                <button><i class="las la-envelope mailButton text-white font-lg bg-ww-blue-600 hover:bg-ww-blue-700 p-1.5 rounded-lg"></i></button>
                            </div>

                        </div>
                    </div>


                    <!--Owner 3-->
                    <div class="p-2">
                        <div class="flex items-center justify-between w-full hover:bg-gray-50 duration-400 rounded-lg p-2">

                            <div class="flex items-center space-x-2">
                                <!-- Grouped user icon and name -->
                                <div class="h-12 w-12 bg-ww-blue-sub-50 rounded-lg mr-2 flex items-center justify-center contactCard">
                                    <p class="text-lg text-ww-blue-sub-500 font-bold">TW</i>
                                </div>
                                <h3 class="font-medium ml-4">Thomas Whitfield</h3><span class="bg-ww-blue-sub-50 companyTag text-xs font-medium mr-2 px-2.5 py-0.5 rounded">Owner</span>
                            </div>

                            <div class="flex space-x-2">
                                <!-- Two buttons grouped on the right -->
                                <button><i class="las la-copy text-gray-600 font-lg border border-gray-200  p-1.5 rounded-lg"></i></button>
                                <button><i class="las la-envelope mailButton text-white font-lg bg-ww-blue-500  p-1.5 rounded-lg"></i></button>
                            </div>

                        </div>
                    </div>
                    <div class="p-4 flex-grow">
                        <!-- Adjusted the padding to p-4 -->
                        <div class="w-full flex justify-end items-center mt-4 mb-auto mr-4">
                            <!-- Adjusted the right margin to mr-4 -->
                            <button type="button" class=" px-3 py-2 text-sm font-medium text-center inline-flex items-center text-white bg-ww-blue-500 rounded-lg  focus:ring-4 focus:outline-none focus:ring-ww-blue-300">
                                <i class="las la-mail-bulk text-xl mr-2"></i>
                                Mail All
                            </button>
                        </div>
                    </div>

                </section>

                <!-- Owner Operator Information -->
                <section class="relative rounded-lg border w-full  border-gray-300 overflow-hidden">
                    <!-- Background Image and Overlay -->
                    <div class="relative top-0 left-0 right-0 h-16 bg-ww-blue-950 bg-cover bg-center z-0">
                        <div class="absolute inset-0 bg-ww-navy-700 opacity-90 clientCardHeadlines"></div>
                        <!-- Headline -->
                        <h2 class="px-4 absolute inset-x-0 bottom-0 h-full flex items-center text-2xl font-bold text-gray-100  z-10">
                            Resources
                        </h2>
                    </div>

                    <!-- Card Content Start-->

                    <!--Item 1-->
                    <div class="p-2 mt-2 duration-500">
                        <div class="flex items-center justify-between w-full rounded-lg p-2">

                            <div class="flex items-center space-x-2">
                                <!-- Grouped user icon and name -->
                                <h3 class="font-medium">County Map</h3>
                            </div>

                            <div class="flex space-x-2">
                                <!-- Two buttons grouped on the right -->
                                <div class="h-16 w-10 px-1 bg-emerald-500 rounded-lg mr-2 flex items-center justify-center relative hoverable-icon-container cursor-pointer">
                                    <p class="font-medium p-1 transition-opacity duration-500 absolute inset-0 flex items-center justify-center">
                                        <i class="lab la-google-drive text-white text-xl original-icon"></i>
                                    </p>
                                    <p class="font-medium p-1 transition-opacity duration-500 absolute inset-0 flex items-center justify-center">
                                        <i class="las la-arrow-alt-circle-right text-white text-xl new-icon opacity-0"></i>
                                    </p>
                                </div>


                                <div class="h-16 w-10 px-1 text-xs bg-gray-100 rounded-lg mr-2 flex items-center justify-center relative hoverable-icon-container cursor-pointer">
                                    <p class="text-xs text-gray-700 font-medium p-1 transition-opacity duration-500 absolute inset-0 flex items-center justify-center original-text">
                                        .SVG
                                    </p>
                                    <p class="transition-opacity duration-500 absolute inset-0 flex items-center justify-center">
                                        <i class="las la-download text-gray-700 text-xl new-icon opacity-0"></i>
                                    </p>
                                </div>
                                <div class="h-16 w-10 px-1 text-xs bg-gray-100 rounded-lg mr-2 flex items-center justify-center relative hoverable-icon-container cursor-pointer">
                                    <p class="text-xs text-gray-700 font-medium p-1 transition-opacity duration-500 absolute inset-0 flex items-center justify-center original-text">
                                        .PNG
                                    </p>
                                    <p class="transition-opacity duration-500 absolute inset-0 flex items-center justify-center">
                                        <i class="las la-download text-gray-700 text-xl new-icon opacity-0"></i>
                                    </p>
                                </div>



                            </div>
                        </div>

                        <!--Item 2-->
                        <div class="mt-2 border-t border-b border-gray-200 bg-gray-100">
                            <div class="flex items-center justify-between w-full duration-400 rounded-lg p-4">

                                <!-- Text all the way on the left -->
                                <h3 class="font-light italic text-gray-600">No Owner Images</h3>

                                <!-- No image card all the way on the right -->
                                <div class="flex items-center space-x-2">
                                    <div class="h-16 w-10 px-1 border border-gray-300 rounded-lg flex items-center justify-center">
                                        <p class="text-xl text-gray-500 font-medium text-center p-2"><i class="las la-user-alt-slash"></i>
                                        </p>
                                    </div>
                                </div>

                            </div>
                        </div>
                </section>
            </div>


            <!--Second row - 3 cards-->

            <!--Card 1 CP Map-->
            <div class="flex flex-row justify-between w-full gap-4 mt-4">
                <div class="basis-1/5 flex-grow mx-auto bg-white rounded-xl border border-gray-300 rounded-lg overflow-hidden ">
                    <div class="relative">
                        <!-- Mapbox Embed without controls and search -->
                        <iframe class="" width="100%" height="400px" src="https://api.mapbox.com/styles/v1/tclmapping/clcgjgy6p001h14pns3s5xsu0.html?title=false&amp;access_token=pk.eyJ1IjoidGNsbWFwcGluZyIsImEiOiJjbDF3YnN1NzgyN3ltM2psbTVxdWNhcjRtIn0.xq9gme4kjVJB1iFpM89x6w&amp;zoomwheel=false&amp;attribution=false&amp;logo=false#7/40.449/-78.054" title="WW Altoona Comp Projects" style="border: none;">
                        </iframe>

                    </div>
                    <div class="p-4 ">

                        <div class="tracking-wide text-lg font-medium mb-2">Store Location</div>
                        <p class="mt-2 text-gray-900 font-medium">
                            <i class="las la-map-marker-alt text-center mb-4"></i>
                            1000 Plank Road, Duncansville, PA 16635
                        </p>

                        <div class="tracking-wide text-lg font-medium my-4">Counties Served</div>
                        <div class="bg-gray-50 justify-between text-gray-600 text-sm p-2 border border-gray-100 rounded-lg">
                            <ul class="flex justify-evenly gap-2 pr-8 p-2 font-medium m-auto flex-wrap">
                                <li><i class="las la-map-marker"></i> Bedford^</li>
                                <li><i class="las la-map-marker"></i> Blair</li>
                                <li><i class="las la-map-marker"></i>Cambria</li>
                                <li><i class="las la-map-marker"></i> Centre</li>
                                <li><i class="las la-map-marker"></i> Clearfield^</li>
                                <li><i class="las la-map-marker"></i> Huntingdon</li>
                            </ul>
                        </div>
                    </div>
                </div>


                <!--Card 2 Web-->
                <div class="basis-1/5 flex-grow border rounded-lg border-gray-300 bg-white">
                    <div class="relative rounded-t-lg top-0 left-0 right-0 h-16 bg-cover bg-center">
                        <div class="absolute inset-0 opacity-90 rounded-t-lg clientCardHeadlines"></div>
                        <h2 class="px-4 absolute  inset-x-0 bottom-0 h-full flex items-center text-2xl font-bold text-gray-100  z-10">
                            Web</h2>
                    </div>
                    <div class="px-4 pt-2 pb-8">
                        <h2 class="text-lg font-semibold mt-2">Landing Pages</h2>
                        <div class="flex flex-row gap-2 mt-4 mb-1.5">
                            <span class="bg-yellow-100 text-yellow-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded flex items-center justify-center">PPC</span>
                            <p>Branded</p>
                            <span class="text-xs ml-auto border border-gray-200 p-1 rounded-lg"><i class="las la-headset"></i> 814-220-6322</span>
                        </div>

                        <div class="flex flex-row gap-2 mt-4 mb-1.5">
                            <span class="bg-yellow-100 text-yellow-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded flex items-center justify-center">PPC</span>
                            <p>Non-Branded</p>
                            <span class="text-xs ml-auto border border-gray-200 p-1 rounded-lg"><i class="las la-headset"></i> 814-220-6337</span>
                        </div>

                        <div class="flex flex-row gap-2 mt-4 mb-1.5">
                            <span class="bg-yellow-100 text-yellow-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded flex items-center justify-center">PPC</span>
                            <p>Competitor Branded</p>
                            <span class="text-xs ml-auto border border-gray-200 p-1 rounded-lg"><i class="las la-headset"></i> 814-220-6339</span>
                        </div>

                        <hr class="my-4 opacity-50">

                        <div class="flex flex-row gap-2 mt-4 mb-1.5">
                            <div class="bg-blue-100 text-blue-600 p-1 px-2.5 rounded-lg font-medium w-fit text-xs flex items-center justify-center">
                                FB</div>
                            <p>Windows</p>
                            <span class="text-xs ml-auto border border-gray-200 p-1 rounded-lg"><i class="las la-headset"></i> 814-467-2114</span>
                        </div>

                        <div class="flex flex-row gap-2 mt-4 mb-1.5">
                            <div class="bg-blue-100 text-blue-600 p-1 px-2.5 rounded-lg font-medium w-fit text-xs flex items-center justify-center">
                                FB</div>
                            <p>Siding & Doors</p>
                            <span class="text-xs ml-auto border border-gray-200 p-1 rounded-lg"><i class="las la-headset"></i> 814-220-6340</span>
                        </div>

                        <hr class="my-4 opacity-50">

                        <h2 class="text-lg font-semibold mt-4">External Pages</h2>
                        <div class="flex align-middle mt-4">
                            <a href="https://windowworldofaltoona.com" class="flex items-center">
                                <!-- Added flex and items-center to align SVG icon and text side by side -->
                                <div class="bg-green-100 p-2 rounded-lg w-12 h-12 flex items-center justify-center mr-4">
                                    <!-- Added margin-right (mr-4) for spacing between SVG and text -->
                                    <img class="h-12" src="img/icons/green_leaf_d.svg" />
                                </div>
                                <p class="ml-2 font-medium">Organic Website</p>
                            </a>
                        </div>

                        <div class="flex align-middle mt-4">
                            <a href="https://windowworldofaltoona.com" class="flex items-center">
                                <!-- Added flex and items-center to align SVG icon and text side by side -->
                                <div class="bg-gray-200 p-2 rounded-lg w-12 h-12 flex items-center justify-center mr-4">
                                    <!-- Added margin-right (mr-4) for spacing between SVG and text -->
                                    <img class="h-12" src="img/logos/Google_G.svg" />
                                </div>
                                <p class="ml-2 font-medium">Google Business Page</p>
                            </a>
                        </div>

                        <div class="flex align-middle mt-4">
                            <a href="#" class="flex items-center">
                                <!-- Added flex and items-center to align SVG icon and text side by side -->
                                <div class="bg-blue-300 p-2 rounded-lg w-12 h-12 flex items-center justify-center mr-4">
                                    <!-- Added margin-right (mr-4) for spacing between SVG and text -->
                                    <img class="h-12 mt-[2px]" src="img/logos/social_facebook.svg" />
                                </div>
                                <p class="ml-2 font-medium">Facebook Business Page</p>
                            </a>
                        </div>
                    </div>
                </div>
                <!-- Card 3 (Global Revisions) -->
                <div class="basis-2/5 relative border rounded-lg bg-white border-gray-300">
                    <div class="relative rounded-t-lg rounded-tr-lg top-0 left-0 right-0 h-16 bg-cover bg-center">
                        <div class="absolute inset-0 opacity-90 rounded-t-lg clientCardHeadlines"></div>
                        <h2 class="px-4 absolute inset-x-0 bottom-0 h-full flex items-center text-2xl font-bold text-gray-100  z-10">
                            Global Revisions</h2>
                    </div>

                    <!--Global Revision 1-->
                    <div class="px-4 pt-4">
                        <div id="alert-additional-content-1" class="p-4 mb-4 text-gray-800 border border-grey-300 rounded-lg bg-gray-50" role="alert">
                            <div class="flex items-center">
                                <!-- Red Circle with Lighter Border -->
                                <span class="flex-shrink-0 w-2 h-2 mr-2 bg-red-500 border-2 border-red-300 rounded-full"></span>

                                <span class="sr-only">Info</span>
                                <h3 class="text-base font-bold">Fake Brand New Revision</h3>
                            </div>
                            <div class="mt-4 mb-6 text-sm">
                                Attention teams, a new product variant is being introduced in our Bethlehem branch.
                                Please make the
                                necessary updates on promotional materials.<br><br>
                                <span class="font-medium">"Introducing the new deluxe espresso machine variant!"</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-xs text-gray-400 italic">
                                    <i class="las la-clock"></i>&nbsp; Last edited: 4:13 PM - 10/2/2021
                                </span>

                                <div class="flex items-center">
                                    <button type="button" class="text-white bg-emerald-600 hover:bg-emerald-600 focus:ring-4 focus:outline-none focus:ring-emerald-200 font-medium rounded-lg text-xs px-3 py-1.5 mr-2 text-center inline-flex items-center">
                                        <i class="lab la-google-drive"></i> View on Sheets
                                    </button>
                                    <button type="button" class="text-gray-800 bg-transparent border hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-200 font-medium rounded-lg text-xs px-3 py-1.5 text-center" data-dismiss-target="#alert-additional-content-1" aria-label="Close">
                                        Dismiss
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!--Global Revision 2-->
                    <div class="px-4 pt-4 mt-2">
                        <div id="alert-additional-content-1" class="p-4 mb-4 text-gray-800 border border-grey-300 rounded-lg bg-gray-50" role="alert">
                            <div class="flex items-center">
                                <!-- Red Circle with Lighter Border -->
                                <span class="flex-shrink-0 w-2 h-2 mr-2 bg-yellow-500 border-2 border-yellow-300 rounded-full"></span>

                                <span class="sr-only">Info</span>

                                <h3 class="text-base font-bold">Altoona - Business Hours Change</h3>
                            </div>
                            <div class="mt-4 mb-6 text-sm">
                                Effective immediately, please make the following change to all print and digital (where
                                applicable) for WW Altoona.<br><br>
                                <span class="font-medium">"Change Saturday hours to by appointment only."</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-xs text-gray-400 italic">
                                    <i class="las la-clock"></i>&nbsp; Last edited: 1:22PM - 9/1/2021
                                </span>

                                <div class="flex items-center">
                                    <button type="button" class="text-white bg-emerald-600 hover:bg-emerald-600 focus:ring-4 focus:outline-none focus:ring-emerald-200 font-medium rounded-lg text-xs px-3 py-1.5 mr-2 text-center inline-flex items-center">
                                        <i class="lab la-google-drive"></i> View on Sheets
                                    </button>
                                    <button type="button" class="text-gray-800 bg-transparent border hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-200 font-medium rounded-lg text-xs px-3 py-1.5 text-center" data-dismiss-target="#alert-additional-content-1" aria-label="Close">
                                        Dismiss
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!--End Dashboard Content-->
                </div>
            </div>
        </div>

        <!--Tab 2-->
        <div class="clientNav-tab-pane" id="clientNav-tab2"><?php include 'client/sub_globalrev.php'; ?></div>
        <!--Tab 3-->
        <div class="clientNav-tab-pane" id="clientNav-tab3"><?php include 'client/sub_offers.php'; ?></div>
        <!--Tab 4-->
        <div class="clientNav-tab-pane" id="clientNav-tab5"><?php include 'client/sub_tracking.php'; ?></div>
        <!--Tab 5-->
        <div class="clientNav-tab-pane" id="clientNav-tab6"><?php include 'client/sub_cred.php'; ?></div>
    </div>
